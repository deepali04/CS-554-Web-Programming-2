...
router.route('/:id/comments').post(async (req, res) => {
  ...
  try {
    ...
    if(await redisClient.exists(`recipes:${req.params.id}`)){
      await redisClient.set(`recipes:${req.params.id}`, JSON.stringify(newComment));
      await redisClient.ZINCRBY('recipeAccessCounts', 1, req.params.id);
      console.log('Comment Updated in the cache');
    }
...
router.route('/:recipeId/:commentId').delete(async (req, res) => {   
  ...
  try {
    ...
    if(await redisClient.exists(`recipes:${req.params.id}`)){
      await redisClient.set(`recipes:${req.params.id}`, JSON.stringify(deletedComment));
      await redisClient.ZINCRBY('recipeAccessCounts', 1, req.params.id);
      console.log('Deletion of comment Updated in the cache');
    }
