...
const createComment = async (recipeId, userDetails, comment) => {
  ...
  const wrongRecipe = await recipeCollection.findOne({ _id: new ObjectId(recipeId)});
...
const removeComment = async (commentId, username) => { 
  ...
  const removeComment = await recipeCollection.updateMany({}, {$pull: {comments: {_id: new ObjectId(commentId)}}});   
  ...
  const recipeReturned = await recipeCollection.findOne({ _id: new ObjectId(recipeID)});
