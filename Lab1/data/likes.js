...
const addLikes = async (userId, recipeId) => {
  ...
  const recipeExists = await recipeCollection.findOne({_id: ObjectId(recipeId)});
  ...
  if(recipeExists.likes.includes(userId)){
    ...
    const removedLike = await recipeCollection.updateOne({_id: ObjectId(recipeId)},{$pull:{"likes":userId}});
    ...
    const addLikes = await recipeCollection.updateOne({_id: ObjectId(recipeId)},{$push:{"likes":userId}});
